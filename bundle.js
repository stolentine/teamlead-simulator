(() => {
  ...
  var ru_default = {
    overlay: {
      intro: {
        ...
        teammate: function(d) {
          return x(d.name, "ru", " nominative".trim()) + " - специалист в " + d.skills;
        }
      },
      editor: {
      ...
      code_review: {
        look_at_code: {
          bad: function(d) {
            return "Вы открыли задачу " + x(d.name, "ru", " genitive".trim()) + " в вечно тормозящем GitPub'e. Код выглядит откровенно плохо";
          },
          not_too_bad: function(d) {
            return "Вы открыли задачу " + x(d.name, "ru", " genitive".trim()) + " в вечно тормозящем GitPub'e. Код смотрится довольно небрежным";
          },
          ok: function(d) {
            return "Вы открыли задачу " + x(d.name, "ru", " genitive".trim()) + " в вечно тормозящем GitPub'e. На первый взляд, код неплох";
          }
        },
      ...
      calendar: {
        ...
        one_on_one: {
          ...
          message: function(d) {
            return "One on One прошел продуктивно. " + x(d.name, "ru", " nominative".trim()) + " поделился своими переживаниями по поводу дедлайна и успехами в работе. Вы поделились кусочком тимлидской мудрости и " + x(d.name, "ru", " nominative".trim()) + " явно остался доволен [+Навык]";
          }
        },
        knowledge_sharing: {
          info: function(d) {
      ...
      teammate: {
        action: {
          help: function(d) {
            return "Помочь " + x(d.name, "ru", " dative".trim()) + " с задачей (1 ч.)";
          }
        },
        helped: function(d) {
          return "Свежим взглядом пробежавшись по задаче " + x(d.name, "ru", " genitive".trim()) + ", Вы довольно быстро обнаружили проблему и мудрым советом помогли ему продолжить работу [+Навык]";
        },
        ...
        cant_talk_after_work: function(d) {
          return "Время уже не рабочее и " + x(d.name, "ru", " nominative".trim()) + " ушел в оффлайн, лучше написать ему позже";
        }
      },
      task: {
        status: {
          ...
          in_dev: {
            ...
            teammate: function(d) {
              return "Над этой задачей сейчас работает " + x(d.name, "ru", " nominative".trim());
            }
          },
          in_review: {
            yours: function(d) {
            ...
            teammate: function(d) {
              return "Задача " + x(d.name, "ru", " genitive".trim()) + " готова и ждет от Вас ревью";
            }
          },
    ...
    chat: {
      action: {
        message_teammate: function(d) {
          return "Написать " + x(d.name, "ru", " dative".trim());
        }
      },
      greetings: {
        v1: function(d) {
  ...
  function contains(x2, y, sx, sy, width, height) {
    return x2 >= sx && y >= sy && x2 < sx + width && y < sy + height;
  }
  function point_to_screen_space(x2, y) {
    const p = point_to_world_space(x2, y);
    return xy(p.x / game.dpi_scale, p.y / game.dpi_scale);
  }
  function point_to_world_space(x2, y) {
    const p = current_context().getTransform().transformPoint({x: x2, y});
    return xy(p.x, p.y);
  }
  ...
  function set_layout_cursor(x2, y) {
  ...
  function path_rounded_rect(x2, y, w, h, r) {
    if (w < 2 * r)
      r = w / 2;
    if (h < 2 * r)
      r = h / 2;
    ...
    path.moveTo(x2 + r, y);
    path.arcTo(x2 + w, y, x2 + w, y + h, r);
    path.arcTo(x2 + w, y + h, x2, y + h, r);
    path.arcTo(x2, y + h, x2, y, r);
    path.arcTo(x2, y, x2 + w, y, r);
    return path;
  }
  function path_circle(x2, y, r) {
    const ctx = current_context();
    ctx.beginPath();
  ...
  function fill_circle(color, x2, y, r) {
    const ctx = current_context();
    ctx.fillStyle = color;
    path_circle(x2, y, r);
    ctx.fill();
  ...
  function draw_messenger(app, width, height) {
    ...
    function draw_status_bubble(x2, y, online, selected) {
      path_circle(x2, y, status_bubble_r);
      if (online) {
  ...
  function draw_wrike(app, width, height) {
    ...
    function draw_block_title(text, x2, y) {
      push_font(18, "bolder");
      ctx.fillStyle = "#000";
      ctx.textBaseline = "top";
    ...
    function draw_block(x2, y, w, h) {
      const rect = path_rounded_rect(x2, y, w, h, 3);
      ctx.shadowBlur = 8;
      ctx.shadowColor = "#eee";
  ...
  function draw_browser(width, height) {
    ...
    website_button(images.icon_web_jabr, "Jabr", 0);
    website_button(images.icon_web_you_cube, "YouCube", 1);
  }
  ...
  function draw_dock(width, height) {
    ...
    ctx.fill(rect);
    let app_index = 0;
    function ease_out_circular(x2) {
      return Math.sqrt(1 - Math.pow(x2 - 1, 2));
    }
    ...
    for (const app of game.apps) {
      ...
      const x2 = 16 + app_index * (64 + 16) - adjust / 2;
      const y = 16 - adjust / 2;
      const w = 64 + adjust, h = 64 + adjust;
      ctx.drawImage(image, 0, 0, image.width, image.height, x2, y, w, h);
      if (notifications > 0) {
        const r = 12;
        const bubble_x = x2 + w;
      ...
      const result = button_behavior(x2, y, w, h);
      if (result == 1) {
  ...
  function draw_overlay_screen() {
    ...
    function draw_menu_background(x2, y, width, height) {
      const overlay_show_time = game.overlay.type == 14 ? 2e3 : 100;
      const rect = path_rounded_rect(x2, y, width, height, 4);
    ...
    function auto_menu(x2, y, width) {
      ...
      return {
        ...
        button(text) {
          ...
          const state = button_behavior(x2, button_y, width, button_height);
          buttons.push({
            text,
            hovered: state == 1
          });
        ...
        finish() {
          ...
          const rect = draw_menu_background(x2, y, width, height);
          push_clip(rect);
          let line_y = y + padding;
          push_font(message_font);
          ctx.textBaseline = "top";
          for (const line of lines) {
            const text_x = center_text ? x2 + (width / 2 - ctx.measureText(line).width / 2) : x2 + padding;
            ctx.fillStyle = "#000";
          ...
          for (const button2 of buttons) {
            if (button2.hovered) {
              ctx.fillStyle = "#ccc";
              ctx.fillRect(x2, button_y, width, button_height);
            }
            ctx.fillStyle = lines.length > 0 ? "#003a57" : "#000";
            ctx.fillText(button2.text, x2 + padding, button_y + button_height / 2, width);
    ...
    switch (overlay.type) {
      ...
      case 3: {
        let draw_standup_avatar = function(img, name, title) {
          const x2 = game.canvas_width / 2 - avatar_side / 2;
          const y = game.canvas_height - 840;
          const ctx2 = current_context();
          const rect = path_rounded_rect(x2, y, avatar_side, avatar_side + 60, 4);
          ctx2.shadowBlur = 8;
          ...
          ctx2.shadowBlur = 0;
          push_clip(rect);
          ctx2.drawImage(img.img, x2, y, avatar_side, avatar_side);
          push_font(18);
          const name_width = ctx2.measureText(name).width;
          const name_x = x2 + avatar_side / 2 - name_width / 2;
          ...
          const title_x = x2 + avatar_side / 2 - title_width / 2;
...
${i18n.overlay.game_over.message.restart()}`);
  ...
  function fix_canvas_dpi_scale(canvas, context, scale = true) {
  ...
  function random_up_to(x2) {
    return Math.floor(Math.random() * x2);
  }
